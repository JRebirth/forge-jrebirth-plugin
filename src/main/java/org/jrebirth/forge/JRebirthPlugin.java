...
import org.jrebirth.forge.helper.ResourceHelper;
...
@RequiresFacet({ DependencyFacet.class, JavaSourceFacet.class,
		ResourceFacet.class, JRebirthFacet.class })
@RequiresProject
public class JRebirthPlugin implements Plugin {
	...
	/**
	 * The setup command for JRebirth. This adds dependency to the current
	 * project
	 * 
	 * @param out
	 *            the out
	...
	public void setup(
			final PipeOut out,
			@Option(name = "module", shortName = "m", help = "The Module name to be installed.") final String moduleName)
		...
		if (moduleName != null) {
			if ("Presentation".equalsIgnoreCase(moduleName)) {
				...
				installDependencies(this.project, this.shellPrompt, out,
						jrebirthPresentationDependency(), true);
			}
		}
	}

	/**
	 * If jrebirth command is not executed with any argument this method will be
	 * called.
	 * 
	 * @param out
	 *            the out
	...
	public void defaultCommand(final PipeOut out) throws Exception {
		...
		if (this.project.hasFacet(JRebirthFacet.class)) {
			ShellMessages.success(out,
					messages.getMessage("jrebirth.is.installed"));
		} else {
			ShellMessages.warn(out,
					messages.getMessage("jrebirth.is.not.installed"));
		}
	 ...
	 * @param name
	 *            the name
	 * @param controllerGenerate
	 *            the controller generate
	 * @param beanGenerate
	 *            the bean generate
	 * @param fxmlGenerate
	 *            the fxml generate
	...
	public void createMVC(
			final PipeOut out,
			@Option(name = "name", shortName = "n", required = true, help = "Name of the UI Group to be created.") final String name,
			@Option(name = "controllerGenerate", shortName = "cg", required = false, flagOnly = true, defaultValue = "true", help = "If true, Controller will be generated for the MVC.") final boolean controllerGenerate,
			@Option(name = "beanGenerate", shortName = "bg", required = false, flagOnly = true, defaultValue = "true", help = "If true, Bean will be generated for the MVC.") final boolean beanGenerate,
			@Option(name = "fxmlGenerate", shortName = "fg", required = false, flagOnly = true, defaultValue = "false", help = "If true, FXML document will be generated for the MVC in resource folder.") final boolean fxmlGenerate
	...
	public void createCommand(
		...
		final int choiceIndex = this.shellPrompt.promptChoice(
				"Which type of Command you like to create ?",
				PluginUtils.COMMAND_TYPES);

		createNonUiFiles(CreationType.COMMAND, commandName, out,
				(String) PluginUtils.COMMAND_TYPES[choiceIndex]);
	}
	...
	public void createResource(
			final PipeOut out,
			@Option(name = "all", shortName = "a", required = true, help = "Generate all the resource") final boolean allResource,
			@Option(name = "colorGenerate", shortName = "cg", flagOnly = true, required = false, help = "Generate resource for Colors") final boolean colorGenerate,
			@Option(name = "fontGenerate", shortName = "fg", flagOnly = true, required = false, help = "Generate resource for Fonts") final boolean fontGenerate,
			@Option(name = "imageGenerate", shortName = "ig", flagOnly = true, required = false, help = "Generate resource for Images") final boolean imageGenerate) {
		createResourceFiles(out, allResource, colorGenerate, fontGenerate,
				imageGenerate);
	}
	...
	public void colorAddGray(
			final PipeOut out,
			@Option(name = "name", shortName = "n", required = true, help = "Name of color object") final String colorName,
			@Option(name = "value", shortName = "v", required = true, help = "Color's value") final String colorValue,
			@Option(name = "colorType", shortName = "t", completer = ColorTypeCompleter.class, required = true, help = "Type of the color you like to add") final String colorType,
			@Option(name = "opacity", shortName = "o", required = false, defaultValue = "1.0", help = "Color's opacity value") final double opacityValue) {

		if (ColorTypeCompleter.isColorTypeExist(colorType) == false) {
			ShellMessages.error(out,
					messages.getMessage("color.type.is.incorrect"));
			return;
		}

		if (ResourceHelper.validateColorValueUsingType(colorType, colorValue) == false) {
			ShellMessages
					.error(out,
							messages.getMessage(
									"color.value." + colorType.toLowerCase()
											+ ".is.incorrect",
									colorType.toLowerCase()));
			return;
		}

		ResourceHelper.manageColorResource(project, shell, out, colorName,
				colorValue, colorType, opacityValue);
	}
	...
	public void appConfig(
			final PipeOut out,
			@Option(name = "key", shortName = "k", completer = AppPropertyCompleter.class, help = "Key name in the property file.") final String key,
			@Option(name = "value", shortName = "v", help = "New value for the key") final String value,
			@Option(name = "showAll", shortName = "a", flagOnly = true, defaultValue = "false", help = "Show all the application properties") final boolean showAll)
			throws IOException {
		...
		try {
			prop.load(resourceFacet.getResource("jrebirth.properties")
					.getResourceInputStream());
		} catch (final IOException e) {
			ShellMessages.error(out,
					messages.getMessage("unable.to.read.properties.file"));
			return;
		}
		...
		if (value != null) {
			...
			if (prop.containsKey(key) == false) {
				ShellMessages.info(out,
						messages.getMessage("given.key.not.found"));
			}

			prop.setProperty(key, value);
			final File fxmlFile = resourceFacet.createResource(new char[0],
					"jrebirth.properties").getUnderlyingResourceObject();
	...
	/**
	 * Creates Java files for user interface mainly for Model, Controller and
	 * View.
	 * 
	 * @param out
	 *            the out
	...
	private void createUiFiles(final PipeOut out, final CreationType type,
		...
		final TemplateSettings settings = new TemplateSettings(
				javaStandardClassName, topLevelPackage);
		...
		if (fxmlGenerate) {
			...
			determineFileAvailabilty(this.project, resourceDir,
		...
		if (beanGenerate) {
			...
			determineFileAvailabilty(this.project, beansDirectory,
					CreationType.BEAN, javaStandardClassName, out, "", ".java",
					settings);
		}
	}
	 ...
	 *            the command type
	 */
	private void createNonUiFiles(final CreationType type,
			final String fileName, final PipeOut out, final String commandType) {
		...
		try {
			...
			final TemplateSettings settings = new TemplateSettings(finalName,
					topLevelPackage);
			settings.setTopLevelPacakge(topLevelPackage + type.getPackageName());
			...
			determineFileAvailabilty(this.project, directory, type, finalName,
					out, "", ".java", settings);

		} catch (final Exception e) {
			ShellMessages.error(out,
					messages.getMessage("could.not.create.file"));
		}
	}
	 ...
	 *            the image generate
	 */
	private void createResourceFiles(final PipeOut out,
			final boolean allResource, final boolean colorGenerate,
			final boolean fontGenerate, final boolean imageGenerate) {
		...
		final TemplateSettings settings = new TemplateSettings(finalName,
				topLevelPackage);
		final Map<String, TemplateSettings> context = new HashMap<String, TemplateSettings>();
		settings.setTopLevelPacakge(topLevelPackage
				+ CreationType.RESOURCE.getPackageName());

		context.put("settings", settings);

		try {
			directory = sourceFolder.getChildDirectory(Packages
					.toFileSyntax(topLevelPackage
							+ CreationType.RESOURCE.getPackageName() + "."));

			createPackageIfNotExist(directory,
					CreationType.RESOURCE.getPackageName(), out);

			if (directory != null
					&& directory.getChild(finalName + ".java").exists() == false) {

				if (allResource || colorGenerate) {
					createJavaInterfaceUsingTemplate(this.project,
							"TemplateColorResource.ftl", context);
				}
				if (allResource || fontGenerate) {
					createJavaInterfaceUsingTemplate(this.project,
							"TemplateFontsResource.ftl", context);
					createJavaEnumUsingTemplate(this.project,
							"TemplateFontNamesResource.ftl", context);
				}
				if (allResource || imageGenerate) {
					createJavaInterfaceUsingTemplate(this.project,
							"TemplateImagesResource.ftl", context);
				}

			} else {
				ShellMessages.error(out,
						messages.getMessage("file.already.exist", finalName));
			}
